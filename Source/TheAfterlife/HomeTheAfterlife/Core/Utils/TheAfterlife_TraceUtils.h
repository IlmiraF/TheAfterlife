...
namespace TheAfterlife_TraceUtils
{
	...
	TArray<FHitResult> SweepCapsuleMultiByObjectType(const UWorld* World, const FVector& Start, const FVector& End, TArray<TEnumAsByte<EObjectTypeQuery>> ClimbableSurfaceTraceTypes);
	FHitResult LineTraceSingleByObject(const UWorld* World, const FVector& Start, const FVector& End, TArray<TEnumAsByte<EObjectTypeQuery>> ClimbableSurfaceTraceTypes);
}